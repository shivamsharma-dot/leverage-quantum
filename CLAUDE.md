...
## 2026-08-30 (later still) — the heading-only title fix over-corrected; auto titles restored, KPI strips promoted, scan cost cut (commits `1f53428`..`b3ae835`)

Every claim below was checked against the deployed build page by page with the Chrome extension, not reasoned about — four of the six commits in this range exist only because a fix that looked right failed when it was actually clicked.

**The regression this fixes.** Restricting auto-slide titles to real `h1`-`h6`/`[role="heading"]` did stop the garbled run-ons, but this app's local cards render their titles as plain divs, so it also deleted almost every title it was meant to protect: all 5 on Summary and 4 of 5 on Revenue (the documented known-good baseline) fell back to "Section N" while the correct name — "Meta Ads", "Collected vs Projected", "Revenue by Source" — sat one element inside the card. Revenue's single card with a genuine heading was the only survivor, which is what proved the diagnosis rather than a guess at it. Titles now fall back to a LEAF element's own text: a leaf has no children, so it cannot reproduce the original failure, which was reading a CONTAINER's `textContent` and gluing label + value + subtitle into "Total People3,397 — Lead — Squared, real-time". Registered Cards were never affected and still pass their titles explicitly.

**Three guards keep the leaf fallback honest,** because a bare "first text-bearing element" is exactly what was tried and abandoned once before. `isTitleish()` rejects values rather than labels, so a tile leading with a currency figure or "3,397" is skipped. `repeatsInCard()` rejects a leaf whose class occurs 5+ times in the card — what a column header looks like. `inRepeatedRow()` rejects a leaf with an ancestor, inside the card, whose siblings share one height — what a row list looks like, with no dependence on `<table>` markup, classes or text. Meta Ads needed all three and exposed two bugs in them live: the header row slipped through first, so the card titled itself "Campaign"; then the first DATA row slipped through as "PMX_FB_Ger_NAS_08July2026", because the row-list test anchored on `kids[0]`, which is the 55px header row, and compared every 71px data row against it. Using the largest equal-height group instead fixed it. That card now reads an honest "Section 2", the intended outcome for a card with no title of its own.

**KPI strips are one slide instead of zero.** A row of 3+ tiles that carry the card signature but sit under the 280x80 floor now gets its container promoted to a single slide. This was previously written off as "Meta Ads #10, not fixable without a bespoke bare-stat-row detector"; that framing did not survive inspection — the five tiles each already had `border-radius: 16px`, a real box-shadow, and sat inside the chosen content root, and only the 280px width floor was excluding them, so no new detector was needed and the whole thing was a much smaller lift than claimed. Meta Ads went 1 slide to 2, Google Ads 2 to 3 (its 8 previously-dropped tiles), Summary 5 to 6, Revenue 5 to 6. The strip is titled "Key metrics", decided by construction when promoted and by equal-width children otherwise; a first attempt tested content ("every tile contains a digit") and was replaced after it lost the race against data loading on Summary and titled the band "MTD Spend" after its first tile.

**Scan cost.** The flat `querySelectorAll('div,section,article')` is now a DFS that stops descending once a card matches, since only outermost cards are ever presented — on Meta Ads the one matching card holds ~7,000 of the page's ~7,900 elements. Testing `isTitleish()` before the structural guards mattered just as much: without it a 480-row table paid for a row-list walk, a repeated-class `querySelectorAll` and a baseline sweep on every numeric cell, which measured 60.8ms live and briefly made this pass a net perf loss. Meta Ads' Present click went 58.3ms to ~16ms (15.8/14.7/18.5 across three clicks); Summary 5.4ms, Team Mapping 3.4ms, Google Ads 10ms, Revenue 10.8ms.

**Also excluded from auto-detection:** `role="alert"`/`role="status"`, and anything class-matched as warning/notice/callout/banner/toast. Revenue's "Unidentified Source Revenue" advisory is 65px tall on desktop and so fell under the height floor by luck, but wrapped past it at phone widths and became a 90%-empty cream slide; confirmed gone at 531px, where Revenue is back to 5 real sections. A floating `position: fixed` subtree is now skipped whole rather than skipping only the container element, and `absolute` is skipped only when it also looks like floating UI (z-index >= 100, or a dialog/menu/listbox role).

**Re-verified live and unchanged by this pass:** exit cleanup leaves zero inline `display:none` and zero elevated leftovers on Summary, Revenue, Meta Ads, Team Mapping and Google Ads; the SnapshotTool trigger is still absent during a presentation (0 in the DOM) and returns after; the single-line title clamp still holds a constant 20px kicker-to-slide clearance with no clipping; `/settings` and `/ask-ai` still return "Nothing to present here yet"; the narrow-width hit box still passes at 531px; registered-Card titles ("Spend by campaign", "All campaigns", "475 people") are untouched.

**Known and deliberate.** The KPI band is still not a slide below ~531px, where the tiles reflow and the container stops qualifying — that is unchanged from before this pass, not a new regression. `#14` (auto-detected content not reflowing to fill its enlarged frame) and the ~400ms transient single-column grid flash remain untouched, per the earlier call not to block on them.

## 2026-08-31 — Organic & Social: multi-account Instagram, one full-width card per account (commit `9fa8976`)

Explicit ask, "connect all our instagram accounts" -- every Instagram business account should get its own card matching the existing single-account layout exactly (title, `@handle · date range`, 3x2 KPI grid), not squeezed into the 2-col grid it used to share with Website.

**The real architectural constraint, confirmed before writing any code**: Instagram Login (the flow `lib/instagram.mjs` already uses, per its own header comment -- a dedicated Meta App isolated from the Ads token, chosen after adding IG scopes to the shared Ads app once broke it in production) issues **one access token per account**, generated at the moment someone logs into THAT account's own username/password via the developer console's "Generate token" tool. There is no "list every Instagram account this token can see" call the way Meta Ads' shared ad-account token has (`graphGetAll` over `/me/adaccounts`) -- each additional account is a genuinely separate credential set, not a bigger list to enumerate from one token. So "connect all our accounts" is bounded by how many accounts' owners are willing to go through the Instagram Tester + accept-invite + Generate-token flow already documented in the `organic-social-analytics-buildout` memory for the first account (@leverageedu).

**What shipped, generalized for N accounts with zero future frontend changes:**
- `lib/instagram.mjs`: new `instagramAccounts()` -- the original account keeps reading the legacy `INSTAGRAM_ACCESS_TOKEN`/`INSTAGRAM_BUSINESS_ID` env vars completely untouched (key `'leverageedu'`, zero migration risk to what was already live), and every account after that is one more entry appended to a new `INSTAGRAM_ACCOUNTS_JSON` env var (`[{"key":"...","businessId":"...","accessToken":"..."}]`), falling back to the shared `INSTAGRAM_APP_SECRET` per entry unless one ever needs a different app.
- `api/crm-leads.js`: new `mode=accounts` lists connected account keys only (no secrets, safe for the browser); `mode=profile`/`mode=insights` take an optional `?account=<key>`, defaulting to the legacy single-account creds when omitted so nothing existing changed behavior.
- `src/pages/OrganicSocialDashboard.jsx`: Instagram pulled out of the Website/YouTube 2-col grid entirely. `load()` now fetches `mode=accounts` first, then fans out a profile+insights fetch per returned key in parallel (`loadIgAccount`), and the page renders one full-width `<Card title="Instagram">` per account in a `.map()` loop -- adding account #2 in the future is purely a `INSTAGRAM_ACCOUNTS_JSON` config change, no code change.

**Live-verified**: `GET /api/crm-leads?source=instagram&mode=accounts` correctly returns `{"accounts":[{"key":"leverageedu"}]}` today (only one account connected so far), and the page renders exactly that one Instagram card, full-width, in the new position below Website/YouTube -- real data unchanged (followers 4,21,772, reach 2,12,46,960, etc.), confirming the backward-compat default path works identically to before this change.

**What's still needed from the user, unavoidably manual per account**: for every additional Instagram business account they want connected, they (or whoever holds that account's login) need to repeat the exact one-time setup already done for @leverageedu -- add the account as an **Instagram Tester** on the "Leverage Quantum Analytics" Meta App (App roles → Add People), accept the tester invite from inside that Instagram account itself (Settings → Apps and websites → Tester Invites), then hit "Generate token" in the developer console (forces a fresh login every time, no session reuse) to get that account's own access token + business ID. Once they have those two values per account, I append one more entry to `INSTAGRAM_ACCOUNTS_JSON` in Vercel and the new card appears with no other code change.

---

## 2026-08-31 -- Organic & Social: removed "Data sources" card, connected 4 Instagram accounts, rebuilt the toolbar as Day/Week/Month/Year, then fixed the header + added Custom range

Four commits across one extended session, none logged at the time -- backfilled now per this file's own standing rule (see the 2026-07-22 "Process note" entry above about exactly this gap).

**Removed the "Data sources" status card** (`406aea4`) -- a green/red connectivity-status strip (Instagram/YouTube/GA4) the user said was "not required in the organic and social page." Deleted `SourceRow`/`StatusChip`/`STATUS_STYLE` and the three `*Status` consts outright, no dead code left behind.

**Instagram expanded from 1 account to 4** (`9fa8976`, then real onboarding over several turns). The mechanism (`instagramAccounts()` in `lib/instagram.mjs`, `INSTAGRAM_ACCOUNTS_JSON` Vercel env var, `mode=accounts` on `api/crm-leads.js`, one full-width `<Card>` per account) is documented in full in the "2026-08-31 -- Organic & Social: multi-account Instagram" entry above -- that entry originally described only the first account (`leverageedu`) and the extensibility mechanism; the mechanism was then actually used to onboard three more real accounts this same session: `leveragecareers`, `ivy100byleverageedu` (key `ivy100`), and `leveragembbs`. Each needed the identical manual, non-automatable real-world steps walked through with the user step-by-step in the Meta Developer Console (My Apps -> "Leverage Quantum Analytics" -> Use cases -> "Manage messaging & content on Instagram" -> Customize -> "API setup with Instagram login"): add the account as an Instagram Tester (App roles -> Add People), accept the tester invite from inside that Instagram account's own Settings -> Apps and Websites -> Tester Invites, then hit "Generate token" (forces a fresh login, no session reuse) to get that account's access token + business ID. Per the user's own explicit security correction ("but we were adding them to environment variables na?"), every token was added directly to the `INSTAGRAM_ACCOUNTS_JSON` Vercel env var by the user themselves -- never pasted into chat, matching the file's existing "credentials live ONLY in Vercel env, repo is public" convention for the first account.

**GA4 access re-verified live** after the user reported granting the `quantum-ga4-reader@leverage-quantum-498506.iam.gserviceaccount.com` service account Viewer role on the GA4 property -- confirmed genuinely unblocked via a real API call, not just trusting the confirmation (matching the `organic-social-analytics-buildout` memory's explicit instruction to always re-test).

**YouTube Analytics API re-verified live** after the Workspace admin (Nishant Bhatia) marked the dedicated OAuth client (`1067014734796-...apps.googleusercontent.com`) as "Trusted" in Admin Console > Security > API controls > Manage third-party app access -- still 403 Forbidden after the grant. Left in an explicit wait-and-recheck state per the user's own "ok lets wait" -- not yet resolved, no further action taken this session.

**Toolbar rebuilt from a single always-visible week-picker into Day/Week/Month/Year presets** (`24f1c9e`), following an explicit "be strict, give me strict advice" ask. The strict call made: split API-driven day-granular sources (Website/GA4, Instagram x N, YouTube) from LinkedIn/X, which are manual weekly-only entries with no "yesterday" to report on -- NOT an "organic vs social" split (that axis doesn't separate anything real, Instagram is exactly as day-granular as GA4). `periodsForGranularity(g)` computes `{curSince, curUntil, priorSince, priorUntil, curLabel, priorLabel}` per preset, each comparing the current PARTIAL period against the SAME ELAPSED PORTION of the prior period (e.g. 3 days into this week vs the first 3 days of last week) -- matching this app's own MTD-vs-prev-month proration convention rather than a misleading partial-vs-complete comparison. `pctDelta()` returns `null` (not a false 0%/divide-by-zero) when the prior value is genuinely zero, or `'new'` when current is nonzero against a zero prior. LinkedIn & X kept their own independent week-by-week `WeekArrow` browser via the `Card` component's `action` prop, completely decoupled from the new toolbar.

**This session's fix** (`017d7d2`), triggered by the user spotting `"This week vs last week · 31 Aug–31 Aug vs 24 Aug–24 Aug"` in the live header and asking "fix this header and where is the custom date range?": (1) the display was mathematically correct (today, 31 Aug 2026, is a Monday, so week-to-date genuinely is a single day) but read as a duplicated/broken string -- added `fmtRange(since, until)` collapsing an identical start/end into one label instead of `"X–X"`. (2) Added a 5th "Custom" pill next to Day/Week/Month/Year, opening the shared `DateRangePicker` component (never a native date input, per house convention) in a popover anchored to the pill row -- backdrop-dismiss and `onClose` both just close the popover with no revert-on-cancel logic needed, since (unlike the reference `GoogleAdsDashboard.jsx` pattern) `granularity` is deliberately only ever set to `'custom'` on a successful "Apply range", never optimistically on opening the popover -- so there is nothing to revert if the user backs out. The prior-period comparison for a custom range is the immediately preceding window of equal length (`lengthDays = curEnd - curStart + 1`, `priorEnd = curStart - 1 day`, `priorStart = priorEnd - (lengthDays-1)`), matching the app's own established "Compare periods" convention elsewhere (e.g. Overall dashboard). Re-opening the Custom pill after an apply pre-fills the previously picked range.

**Live-verified end-to-end**: clicking Week showed the corrected `"This week vs last week · 31 Aug vs 24 Aug"` (no more duplicated date); clicking Custom opened the two-month calendar positioned under the toolbar; picked 2026-08-08 to 2026-08-15, Apply range updated the pill to `"2026-08-08 → 2026-08-15"`, the subtitle to `"Selected range vs previous period · 08 Aug–15 Aug vs 31 Jul–07 Aug"` (8-day window, prior window correctly also 8 days ending the day before), and every card (Website/GA4, both Instagram accounts checked) refetched with real, distinct data for that exact window -- confirmed by different numbers than the "This week" view moments earlier (e.g. Website Organic Users 4,071 -> 43,080). Re-clicking the applied Custom pill correctly reopened the popover with 08-08/08-15 pre-filled and highlighted on the calendar. Clicking Day afterward cleanly reverted the pill to plain "Custom" (no stale range shown) and the header to `"Today vs yesterday · 31 Aug vs 30 Aug"`. Zero app console errors (only the pre-existing benign extension "message channel closed" noise this file documents elsewhere).

---

## 2026-08-31 (later) -- Organic & Social: full Instagram wiring -- growth, breakdowns, audience, top posts

Direct follow-through on "what else can be fetched from Instagram" -- every candidate metric was tested live against the real @leverageedu account (not guessed from docs) before building anything.

**Verification pass first.** Batch-tested every account-level metric name via the existing generic `mode=insights&metrics=...` passthrough; Instagram's own validation error, triggered by one deliberately-misspelled metric, returned the FULL authoritative list of valid names for this API version -- used that as ground truth rather than recalled documentation. Confirmed working with real data: `follower_count` (daily net follower change, the actual fix for the "why no Followers comparison" gap from the previous entry), `likes`/`comments`/`shares`/`saves`/`replies`, `website_clicks`, `profile_links_taps`, `views`. Confirmed genuinely empty for this account across multiple window widths (not wired): `content_views`, `follows_and_unfollows`. Confirmed real Threads metrics (`threads_likes`/`threads_views`/`threads_replies`) error outright and `threads_followers` returns empty -- this account has no real Threads activity, so nothing was built for it. Confirmed demographics metrics (`follower_demographics`, `reached_audience_demographics`, `engaged_audience_demographics`) return metadata with NO value at all unless a `breakdown=country|age|gender` param is sent (silently empty otherwise, not an error) -- tested `breakdown=country` and the combined `breakdown=age,gender` and both work. Confirmed per-post insights use a COMPLETELY DIFFERENT metric vocabulary than account-level (`impressions, shares, comments, likes, saved, replies, total_interactions, navigation, follows, profile_visits, profile_activity, reach, ig_reels_video_view_total_time, ig_reels_avg_watch_time, views, ...` -- again read straight from Instagram's own validation error), tested live on a real Reel (reach 9,797 / likes 53 / views 15,108 / avg watch time 5,520ms). Confirmed **Business Discovery does not work under this app's Instagram-Login connection** -- real error `"Tried accessing nonexisting field (business_discovery)"` when tested against a real public account -- so no UI was built for competitor lookups.

**Backend (`lib/instagram.mjs`, `api/crm-leads.js`, commit `6dde162`):** `fetchInstagramProfile` now also pulls `biography`/`website`/`profile_picture_url`. `fetchInstagramInsights` gained an optional `breakdown` param, forwarded to the Graph API call (required by demographics, silently ignored otherwise). New `fetchInstagramMedia` (recent posts list -- Instagram's `/media` edge has no since/until filter, so callers filter client-side by each post's own `timestamp`), `fetchInstagramMediaInsights` (per-post insights, raw metric passthrough since the valid set differs by media type), `fetchInstagramBusinessDiscovery` (built, tested, confirmed non-functional under this connection type -- left in place with an accurate comment rather than deleted, matching this file's own convention of leaving tested-but-dormant code documented). Three new `handleInstagram` modes: `media`, `media_insights`, `business_discovery`.

**Frontend (`src/pages/OrganicSocialDashboard.jsx`, commit `3175a0b`):** each Instagram account's card gained: a profile-picture avatar + bio + clickable website link above the KPI grid (kept OUT of the `Card` component's `title` prop deliberately -- `Card` uses `title` as a Present-Mode slide-registration key inside a `useEffect([title])`, and a JSX element is a new object reference every render, which would have silently re-registered the slide on every single render); a new **Net Follower Growth** KPI card (`follower_count` time-series summed per period, the actual delta fix -- the plain Followers card stays a delta-less running total, since a snapshot total and a period-over-period growth rate are different questions and conflating them under one card would have been more confusing, not less); 8 more KPI cards for Likes/Comments/Shares/Saves/Replies/Website Clicks/Profile Link Taps/Content Views, all with the same delta-vs-prior-period treatment as the existing cards; a new **Audience** section (lifetime snapshot, explicitly labeled as not period-comparable) showing top 8 countries as a horizontal bar list (country codes resolved to real names via `Intl.DisplayNames`, not a hand-rolled map) and an age-distribution bar list + gender-split percentages (both derived from the one combined `breakdown=age,gender` call, aggregated down to a single dimension each rather than rendering the full 21-cell matrix); and a new **Top Posts** section -- ranks the period's posts by the free like+comment counts already in the media-list response (zero extra API calls), then fetches full insights for only the top 5, bounding the added Graph API calls to at most 5 per account regardless of how many posts the account made in the selected window. Per-post metric set branches on media type (`views`/watch-time metrics only requested for VIDEO/REELS, confirmed live against a real FEED image post rendering correctly with just reach/likes/comments and no view count).

**Live-verified end-to-end** on the real production site, both Day and Month granularity, both `@leverageedu` and `@leveragecareers` accounts: all 15 KPI cards render with real, distinct numbers per account; Net Follower Growth correctly shows "+0" with no delta pill on a day where the metric is genuinely zero (not a false 0%); Audience section renders real country/age/gender breakdowns matching the direct API tests byte-for-byte (India 3,50,476 dominant, 59% Male/26% Female/16% Unspecified); Top Posts (MTD) correctly ranked 5 real posts by engagement including one non-video FEED post (correctly omitting the views/play icon for it), with "15 posted this period" matching the real count. Zero app console errors (only the pre-existing benign extension "message channel closed" noise this file documents elsewhere). No emojis used anywhere in the new UI, per the standing house rule -- the per-post stat row uses the existing SVG icon set instead of the emoji glyphs an early draft had.

## 2026-09-01 — Organic & Social: full information-architecture and visual redesign (commits `249029f`..`b8b3b0e`)

The page worked and every number on it was real, but it had never had a design pass. It was a ~750-line wall of roughly 60 near-identical KPI tiles: the same 15-card block repeated verbatim four times, once per Instagram account, all four stacked full-width; not a single chart anywhere; the Audience section hand-rolling its own flexbox bar lists instead of using the shared `RankedBars`; and no way at all to see a trend over time or compare two accounts against each other. Every figure was a point-in-time snapshot with a delta badge next to it. The brief was to rebuild it around the question it is actually meant to answer — "how is each channel growing, and why" — rather than around the shape of the API responses.

**What changed structurally.** The four stacked account cards collapsed into **one card with an account switcher** (shared `Button`, `size="sm"`, one per handle, plus a live follower count under each). That single move cut the page from four repetitions to one and made cross-account comparison a click instead of a scroll. Above it sits a new **glance strip** — a 3x2 auto-fit grid of `PremKPI` tiles covering the combined cross-channel picture (website organic, total sessions, organic share, combined IG reach, combined followers, YouTube views) so the top of the page answers "how are we doing overall" before you pick an account. Below the switcher, the per-account view now leads with **two Recharts trend charts** rather than tiles: a reach-over-time area chart and a net-follower-growth bar chart, both built from `toDaySeries` over the already-fetched daily arrays. The Audience section was rewritten onto the real shared `RankedBars` from `dashboardKit`, deleting the bespoke bar implementation entirely. Top Posts and the LinkedIn & X week browser kept their logic and were restyled to the card spec.

**Zero data-layer change.** This was strictly a presentation pass and it is worth being precise about that, because the temptation to "just add one more call" while restructuring is strong. `loadIgAccount`, `periodsForGranularity`, `pctDelta`, `extractIgMetrics` and the top-posts ranking are byte-for-byte identical. The redesign issues **exactly the same network requests as before** — the trend charts are derived from daily arrays that were already being fetched and previously thrown away after being summed. The only additions above the `loading` early-return are two pieces of view state, `selectedIg` and `cmpMetric`, deliberately placed there rather than lower down so they do not become conditional hooks.

**Two deliberate deviations from DESIGN_SYSTEM.md, documented rather than left silent.** (1) The follower-growth chart uses two hues within one series — green for positive bars, navy for negative. That breaks the one-series-one-hue rule, but here the hue encodes the *sign*, which is genuine information rather than decoration; direction is also redundantly encoded by the zero line, it stays inside the brand palette, and it avoids reaching for the reserved red. (2) `GranPill` was kept rather than swapped for the shared `Button`, because §3.5 makes date-preset pills their own documented pattern. `Button` is used for Refresh and for the account switcher, where it belongs.

**Three real bugs, all caught only by the live pass — none by the build.** Worth recording because the same trap will come around again:

- **`brandColor is not defined` shipped to production with a completely green `npm run build`.** The `dashboardKit` import line still read `BRAND_RAMP, sourceColor` from the old code and never picked up `brandColor`. Vite/esbuild does not object to a free identifier, so the bundle built clean and the page white-screened on load. Fixed in `2422ab6`. **A passing build does not mean the page renders — always do the live pass.**
- **Truncated KPI labels** ("WEBSITE ORGANI…", "NET FOLLOWER G…", and a YouTube value clipped to "3,85,25…"). Fixed in `0b92bed` by moving the strips to auto-fit `minmax` grids and shortening the label to "Follower Growth", then in `b8b3b0e` by widening the min track from 250px to 280px — at 250px a fourth column could squeeze in at common desktop widths and reflow the glance strip into a ragged 4+2.
- **The account switcher's active state was invisible**, because `primary` and `secondary` are nearly indistinguishable at `size="sm"`. Fixed in `0b92bed` with an explicit navy→blue gradient passed through `style`, which spreads last in `Button`'s `base` and so overrides the variant.

One non-obvious constraint that shaped the markup: `Card`'s `title` prop is the **Present-mode slide-registration key** inside a `useEffect([title])`. It has to stay a stable string. The account card therefore keeps `title="Instagram"` with the handle carried in `sub` — passing JSX or an interpolated handle would silently re-register the slide on every render.

**Live-verified on production** across Day, Month and Custom granularity, with every figure cross-checked against its pre-redesign value; only presentation moved. Website 4,072 organic / 22,853 sessions / 17.8% organic share (the session count had drifted +2 against the baseline as the day's GA4 accrued, as expected). YouTube 4,78,000 views / 3,85,25,430 impressions / 1,467 — exact. `@leverageedu` audience exact on all eight countries (India 3,50,476, Nigeria 10,298, UK 5,346, Pakistan 4,626, US 4,126, UAE 3,920, Indonesia 3,530, Canada 3,400) and on the 59/26/16 gender split. `@leveragecareers` checked as the second account: 19,568 followers, 68,179 reach, India 19,245 — exact. The glance strip's combined IG reach of 17,18,414 reconciles internally as 16,45,266 + 68,179 + 3,225 + 1,744. On Month, both trend charts render 31 points, Top Posts ranks 5, the count reads "15 posted this period", and `@leveragembbs` correctly moves from 4th on Day to 3rd on MTD reach. LinkedIn & X still shows "No entry for this week yet" for 24–30 Aug on both networks. Dark theme checked via `data-theme="dark"` — everything resolves through tokens, no hardcoded hex leaked in.


## 2026-09-01 (later) — Organic & Social: a Year/YTD pass caught three defects the Day/Month verification could not (commits `90cdfa0`, `c4216ee`)

The redesign above was verified at Day and Month, where every figure is five or six digits. Switching the toolbar to Year exposed a whole class of defect that shorter periods hide: at YTD the reach numbers are ten digits in Indian grouping, and three fixed-track grids simply could not hold them. The Website card's `repeat(3,1fr)` clipped `24,23,745` and `66,77,208` to ellipses, and the primary per-account strip's `repeat(5,1fr)` clipped `1,61,23,81,222`. Separately the growth-in-reach `YAxis` was 46px wide with a plain `Math.round(v) + '%'` formatter, so a five-digit percentage rendered as a left-clipped `0000%` — not obviously broken, just quietly meaningless, which is worse.

All three grids moved to `auto-fit` `minmax` so they drop a column rather than ellipsise a number, and the axis went to 62px with `compactN` above 1000. The minimums were measured rather than guessed, and that mattered: the first attempt at `minmax(150px, 1fr)` still clipped, because the resulting 161px tile leaves a 124px value box against a 125px string at the 25px value font. Reading `scrollWidth` against `clientWidth` on the live page gave the real figures, and the second attempt — 180px for the Website card, 220px for the primary account strip — came out clean. A page-wide sweep for any leaf element whose `scrollWidth` exceeds its `clientWidth` under `text-overflow: ellipsis` now returns nothing on this page. The secondary "everything else" strip was deliberately left at 160px, where it measured clean at its smaller font, so it keeps its compact six-across rhythm.

The habit worth keeping from this: **verify at the widest granularity, not just the default one.** Every one of these was invisible at Day, and a green build says nothing about any of them. A pleasant side effect is that the Website card now falls to a 2+1 layout that mirrors the YouTube card beside it, which reads better than the previous 3-up against 2+1 mismatch; the per-account strip likewise settles at 4+1 on a 1400px viewport and returns to 5-up on wider monitors.

Re-verified live at YTD afterwards. `@leverageedu` reach `1,61,23,81,222` and `@leveragecareers` reach `22,70,97,065` agree exactly between the comparison bars and the per-account KPI card, and the four accounts sum to the `1,83,97,19,602` shown in the glance strip — a real internal cross-check, not just a screenshot. Website `24,23,745` organic of `66,77,208` total at 36.3%, YouTube `4,78,000` / `3,85,25,430` / `1,467`. Top Posts ranks 5 of 50 posted in the period, the Audience bars, the LinkedIn & X week browser and the dark theme were all re-checked and are unaffected.

---

## 2026-08-31 (later still) -- LeadSquared Create Opportunity: honest History counts, Bulk import locked to Opportunity ID

Two fixes, both explicitly requested after the user asked "how to fix this problem, i want to see exactly how many i've pushed?" (referring to the History tab's own "Showing the most recent 5,000 of 6,532 total attempts" truncation banner) and separately asked that Bulk import's match-by field "should always be default and mandatory" as Opportunity ID.

**Real total/success/duplicate/failed counts, not silently capped (`api/crm-leads.js`, `src/pages/LeadSquaredDashboard.jsx`, commit `d5c5c7e`).** `OpportunityHistoryTab`'s stats strip was recomputed client-side from `rows` -- and `listOpportunityActivity()` deliberately caps `rows` at 5,000 (5 pages of 1,000, Supabase's own per-request ceiling), a real limit the account has now exceeded (6,532 real logged attempts). So "Total attempts"/"Success"/etc were silently reading off only the most recent 5,000, undercounting by however many older rows fell outside that window, with the truncation banner's OWN suggested fix ("Narrow the date range or Activity Type") not even being real -- `OpportunityHistoryTab.load()` has zero filter controls, that text is a generic `TruncationNote` string reused verbatim from a different, unrelated tab (the raw LeadSquared "Activities" tab, which genuinely does have those filters). Fixed by adding `countOpportunityActivity()` -- a cheap count-only Supabase query (`Range: 0-0` + `Prefer: count=exact`, same technique the existing grand-total already used) run once per status (success/duplicate/failed) ONLY when the list is actually truncated (no extra cost on a normal-sized account). `listOpportunityActivity()` now returns `counts: {total, success, duplicate, failed}` alongside the existing `rows`/`total`/`truncated`, and the frontend's `stats` useMemo prefers these authoritative counts over the client-computed ones whenever present. "Bulk batches" is left as a client-computed lower bound (getting an authoritative distinct-batch count would need a real GROUP BY query, out of scope) but is now honestly labeled with a `*` and an explanatory line when truncated, rather than silently under-reporting with no indication.

**Bulk import locked to Opportunity ID, Single mode untouched (commit `d5c5c7e`).** `searchByAttr` is one shared state across the whole `CreateOpportunityTab` (Single + Bulk both read/write it via the same `SEARCH_BY_OPTIONS` Dropdown), so this needed care to scope correctly: added `useEffect(() => { if (mode === 'bulk') setSearchByAttr('OpportunityID') }, [mode])`, and the render now shows a fixed, non-editable "Opportunity ID (direct update -- no lead matching)" pill (with a "FIXED FOR BULK" badge) instead of the Dropdown whenever `mode === 'bulk'` -- Single mode keeps the full live Dropdown with all 5 options, unaffected, since switching mode only forces the state forward, never reverts it back on returning to Single. Reasoning: real usage of bulk import has consistently been "I have a file of known Opportunity IDs to update" (confirmed by the very History screenshot that prompted this -- a 14,708-row CSV import, all matched by Opportunity ID direct-update), and Capture Opportunities' lead-matching/create-a-new-lead behavior is the wrong tool for a bulk file -- a single typo'd match value could silently create a brand-new lead+opportunity instead of failing loudly, across potentially thousands of rows. The existing sample-CSV/column-mapping/confirmation-dialog/History-logging logic already correctly branches on `isDirect` (confirmed by reading it, not guessed), so locking bulk mode to always be direct-update needed no changes anywhere else.

**Live-verified end-to-end**: Bulk import tab shows the locked "Opportunity ID (direct update -- no lead matching)" pill with no dropdown; switching to Single mode shows the full live Dropdown (chevron present, still `OpportunityID` selected since the shared state carries forward, not reverted); History tab now reads **Total attempts 6,532 / Success 6,530 / Duplicate 0 / Failed 2 / Bulk batches\* 1**, with the new caveat line explaining the real-vs-shown distinction, directly answering "how many have I actually pushed" -- previously this would have shown a capped, wrong "5,000" total. Zero console errors.

---

## 2026-09-02 -- LeadSquared Create Opportunity: "All bulk imports" ships, confirms the fix works end to end

Direct follow-through on "my last bulk imports are missing or lost" -- diagnosed live via a temporary read-only diagnostic mode (`opportunity_activity_diag`, added, used once, removed same session per this file's own convention for one-off investigations): the oldest rows in `leadsquared_opportunity_activity` were the user's own real Aug 27 tests (`testopp.csv` variants, `pasted rows`, `logging-fix verification`), and every one of the newest 50 rows belonged to a single massive `PMX_PerformanceMax_Germany_Footwork_QL_5Dec25.csv` batch run today -- confirming those earlier real imports had aged entirely out of the 5,000-row-capped History list, not lost, just invisible.

Built `listOpportunityBatches()` (`api/crm-leads.js`, new `opportunity_batches_list` mode) exactly as proposed: a narrow scan (`batch_id`/`batch_label`/`created_at` only) pages past the 1000-row PostgREST ceiling to find every distinct batch ever run regardless of age, then 4 parallel count-only queries per batch (`countOpportunityActivity`, extended with an optional extra filter so the same helper serves both the top-level and per-batch counts) give its real total/success/duplicate/failed. Keys on `batch_id` (a real random UUID) falling back to `batch_label` for older rows predating that column -- mirrors `groupOpportunityRows`' own grouping key exactly, so the two can't disagree about what counts as one batch.

New "All bulk imports" section in `OpportunityHistoryTab` (`src/pages/LeadSquaredDashboard.jsx`) renders every batch found this way, permanently, regardless of the 5,000-row cap on the row-level list below it -- a batch whose individual rows are still in that capped window stays expandable (reuses the existing `OpportunityActivityRow`); one that's aged out shows an explicit "individual rows have aged out of the list below -- counts here are still exact" note instead of silently vanishing. The stats strip's "Bulk batches" figure now reads from this same authoritative source too, replacing the old lower-bound guess.

**Live-verified end-to-end, with a real internal reconciliation check**: all 7 real batches now list permanently -- `PMX_PerformanceMax_...csv (14708 rows)` 5,474 total (expandable, real rows render correctly with status pills and request/response detail), `PMX_...csv (1 row)` 1 total, `testopp.csv (1050 rows)` 1,050 total, `testopp.csv (3 rows)` 3 total, `logging-fix verification (1 row)` 1 total/0 success, `pasted rows (1 row)` 2 total/1 success, `testopp.csv (1 row)` 1 total -- every one aged-out correctly labeled, sum (5474+1+1050+3+1+2+1 = 6,532) matching the Total Attempts stat exactly. Zero console errors.

## 2026-09-02 (later) -- New "Apps" dashboard: a 5th BigQuery cache table, once-daily sync, no live BigQuery on page load (commits `05698fe`, `8cc0b2d`)

User added a new saved query, "Apps", in Settings > Data > BigQuery Console -- a join across `direct_monthly_apps`, `enrolled_students_funnel_v2`, a deduped `source_attribution_v3` (via `ROW_NUMBER() OVER (PARTITION BY prospectid ORDER BY opportunity_created_date DESC)`), and `coach_referral_intake_funnel` -- and asked for it as a real page/dashboard, refreshed once a day at 9am IST only, explicitly "no real time refresh."

**Architecture, mirroring `leverage_careers_daily`/`leverageCareersCache.js` exactly** (the established pattern for a BigQuery-backed page with zero live BigQuery calls on page load):
- `supabase/sql/apps_feed_setup.sql` -- new `apps_feed` table, `row_key TEXT PRIMARY KEY` (md5 of `user_id_uuid|opportunity_id|prospect_id|ordinal`, an ordinal disambiguator included since the join's LEFT JOINs make exact duplicates structurally possible) plus 16 data columns (destination country/group/original, intake category/date, school/course name, first app month/date, prospect/opportunity id, source, futwork human/ai, first campaign name) + `sync_id`/`synced_at`, 6 indexes, RLS disabled to match every other table the anon key reads.
- `.github/workflows/apps-sync.yml` -- cron `'30 3 * * *'` (9:00 AM IST, once daily, no other schedule) + `workflow_dispatch`, `timeout-minutes: 10`, a plain `curl -X GET .../api/crm-leads?source=bigquery&mode=apps_sync -H "x-cron-secret: ..."` -- same `CRON_SECRET` repo secret every other cron already uses, no new secret needed.
- `api/crm-leads.js` -- new `mode === 'apps_sync'` branch on the existing `handleBigQuery` dispatcher (no new Vercel function, still 12/12): runs the full unfiltered `APPS_SQL` (this is a full-snapshot sync, not date-windowed, since the query has no since/until param), upserts in batches of 500 stamped with a fresh `sync_id`, then prunes any row not touched by the run -- wholesale-replace, same as `careers_sync`. Cron auth bypass added to the top-level `handler()`, checked before `getSessionUser`, mirroring the existing `careers_sync` bypass line-for-line.
- `src/lib/appsCache.js` -- client-side reader, same keyset/cursor pagination as every other cache reader in this app (PostgREST caps every response at 1000 rows regardless of `limit=`; paging without an explicit stable `order` is a second, independent bug both fixed here by ordering on `row_key`).
- `src/pages/AppsDashboard.jsx` (new) -- KPI row (Total Applications, Distinct Students, Distinct Destinations, Human QL'd, AI QL'd, Still on Floor), 4 `RankedBars` cards (top destinations, by source, top campaigns, by intake category), a Recharts monthly-volume bar chart, and a filterable/searchable/paginated records table with `ExportButton`. Deliberately has NO Day/Week/Month/Year toolbar or Refresh-triggered live fetch, per the explicit "no real time refresh" instruction -- shows a static "Synced [timestamp] · once a day, 9am IST" tag instead.
- Wired into `src/lib/pageList.js` (`id:'apps'`, non-admin, right after Leverage Careers), `src/lib/routePrefetch.js`, `src/App.jsx` (lazy import + route + `PAGE_TITLES`), `src/components/Sidebar.jsx` (new `AppsIcon` glyph + NAV entry + `ICON_MAP`).

**Real bug found on the first live run, fixed same session (`8cc0b2d`):** the manually-triggered first sync failed outright -- `BigQuery: Query exceeded limit for bytes billed: 20000000000. 36346789888 or higher required.` The join's dedup window function over `source_attribution_v3` scans far more than `careers_sync`'s own query, so the shared 20GB `maxBytes` ceiling (copy-pasted from that sibling function) was too tight for this specific query. Raised `apps_sync`'s own `maxBytes` to 60GB (well under the server-side `BQ_BYTES_CEILING` safety valve in `lib/bigquery.mjs`) and left a comment explaining why this one differs from its sibling.

**Live-verified end-to-end.** Re-triggered the workflow after the fix deployed -- run succeeded (1m14s, full scan). Confirmed via a direct Supabase REST count (`Range:0-0` + `Prefer:count=exact`) that exactly 6,629 real rows landed in `apps_feed`. Loaded `/dashboard/apps` live: header reads "Synced 02/09/2026, 13:58:23 · once a day, 9am IST"; Total Applications KPI reads 6,629, matching the Supabase count exactly; Distinct Students 6,626; Distinct Destinations 7; Human QL'd 2,317 (35.0%); AI QL'd 40 (0.6%); Top destinations (UK 4,558/68.8%, Others 1,202, GER 551, ...), By source (Facebook/Google/Content+Brand/Referral/Affiliate/Remarketing/Offline/Bing), Top campaigns (referral_dashboard 1,089/16.4%, ...), By intake category (Sep'2026, Jan'2027, ... -- confirmed these are the sheet's own real intake-season labels, not a mislabeled column), the 12-month application-volume bar chart, and the paginated records table all render real data. Zero app console errors (only the pre-existing benign browser-extension "message channel closed" noise this file documents elsewhere).

**Not yet exercised:** waiting for tomorrow's real 9am IST scheduled cron run (vs. today's manual `workflow_dispatch` trigger) to confirm the schedule itself fires correctly end to end.

## 2026-09-02 (later) -- RankedBars redesigned app-wide: flat single-hue inline bar, no gradient (commit `3398bc3`)

User flagged the "Top Destinations"-style ranked-bar-list widget (`RankedBars` in `dashboardKit.jsx`, feeding 13 pages) as outdated and asked for a market-researched redesign. Four rounds of exploration, each shown as a static-HTML Artifact comparison rendered against the real Apps-dashboard data (UK 4,558/68.8%, Others 1,202/18.1%, GER 551/8.3%, IRE 179/2.7%, CAN 59/0.9%):

1. **Premium gradient pass** (gradient rank chips + glow shadows + tiered medallions, drawing from the app's own `PremKPI` card language) -- rejected: "too much... not gradient everywhere."
2. **Minimal pass, grounded in real Dribbble research** (checked live shots tagged clean/minimal -- a "Space" fintech dashboard's country-list widget, a "Fikri Store" product list) -- two flat single-hue candidates: A (thin underline, no rank number) and B (inline flat bar: label/bar/value/percent on one line). User: "earlier ones were fine" but the two read as interchangeable.
3. **Four structurally distinct concepts** (typographic list / segmented step-meter / real data-table / radial ring), each grounded in a fresh Dribbble reference -- rejected outright: "no not these, earlier ones were fine."
4. Asked directly which of A/B to ship -- **B, Inline flat bar** confirmed via `AskUserQuestion`.

**Shipped** (`src/ui/dashboardKit.jsx`): `RankedBars` rewritten to variant B's exact spec -- one line per row (optional muted rank number -> label, ellipsis-truncated at `maxWidth:'38%'` -> a thin 4px flat bar, single hue, no gradient -> bold tabular value -> muted percent), row gap widened 9->12 for the airier minimal look. Width floor `Math.max(2, raw)` for any nonzero count so a tiny share still shows a sliver (same convention the exploration mockups used). Track background switched from a hardcoded `#F1F5F9` to the theme-aware `var(--bg3)` token -- a real fix, not just style: the old value never adapted to dark/navy/stone. `showRank`'s old bold colored square badge became a small muted number, consistent with the flat direction; `colorFn`/`color`/`total`/`max` props all unchanged, so every one of the 13 call sites needed zero changes.

**Folded in the documented near-duplicate** (`src/pages/LeadQualificationDashboard.jsx`, flagged as a known duplicate since the 2026-06-18 "dashboardKit RankedBars undefined RAMP" entry): deleted its local `RankedBars` + the now-orphaned `const RAMP` array, imported the shared component instead. Bonus correctness fix caught in the process: the local copy's fallback (`colorFn ? colorFn(i) : RAMP[i % RAMP.length]`) walked the brand ramp **by row index** whenever no `colorFn` was passed -- exactly the rank-encodes-color anti-pattern the shared component had already been fixed against elsewhere (2026-08-04 "Chart colour unified" entry) but this duplicate never received. All 3 of this file's call sites (`countryBar`, `degreeBar`, `dispositionBar` on Human/AI QL Detail-style breakdowns) never passed `colorFn`, so they were unknowingly rendering a different rainbow-by-position color on every row in production -- now correctly one flat navy hue like every other page.

**Live-verified after deploy**: `/dashboard/apps`'s "Top destinations"/"By source"/"Top campaigns"/"By intake category" cards all render the new flat single-line bars with real data, matching the originally-screenshotted widget's data but in the new style. `/dashboard/lq-ops`'s "Call disposition" card (the folded-in duplicate) renders identically in the same new style with real numbers (Discover Future Intent 107/28.6%, ...), confirming the fold-in works and the old per-row rainbow coloring is gone. Zero console errors on either page. `npm run build` passed clean before push.

## 2026-09-03 -- LeadSquared Create Opportunity: bulk updates now survive the screen turning off (commit `eff2cec`)

User reported the bulk-import flow for Create Opportunity had failed twice with the same root cause: "if the screen goes off or the system become inactive the opportunity updation stops there / ideally it should work in background or on the server." Explicitly asked for an explanation-only pass first ("dont implement anything now, tell me how you'll solve this"), then walked through several short Q&A rounds before approving ("ok implement this").

**Why it was breaking**: `runOppImportInBackground` ran the whole bulk loop as a plain JS loop inside the browser tab, awaiting each row's full **synchronous** LeadSquared call one at a time. A backgrounded/sleeping tab throttles or suspends its own JS timers, so a long-running loop like this simply stalls mid-file with no error, no completion, and no way to tell how far it got.

**The fix, using LeadSquared's own Async API + Status API rather than a new custom job-queue** (the user's explicit "leadsquared own architecture not supabase nothing" framing, read as "no NEW database/job-queue system" -- the existing `leadsquared_opportunity_activity` table is reused, not replaced): LeadSquared's Async endpoint (`/opportunity/update` on a region-specific host, e.g. `asyncapi-in21.leadsquared.com`, auth via an `x-api-key` header plus the usual `accessKey`/`secretKey` query params) just QUEUES the update and hands back a `RequestID` immediately -- LeadSquared itself then retries the actual work internally up to 10 times over 3 hours (30s, 2m, 7m, 15m, 30m, 45m, 60m, 1.5h, 2h, 3h) if it hits a transient failure, completely independent of whether our browser tab or server function is still running. A separate Status endpoint (`/opportunity/update/status?RequestID=...`) is polled later to find out what actually happened.

**Backend (`api/crm-leads.js`)**: added `leadsquaredAsyncCreds()`/`leadsquaredAsyncRequest()`/`updateLeadSquaredOpportunityAsync()`/`getLeadSquaredOpportunityAsyncStatus()` alongside (not replacing) the existing sync `updateLeadSquaredOpportunity`. Two new dispatch modes on `handleLeadSquared`, both admin-only via `LEADSQUARED_WRITE_MODES`:
- `update_opportunity_bulk_async_submit` -- takes the full row list + a client-generated `batchId` (stable across retries, so resubmitting after a partial run is idempotent: it first queries which rows for that `batchId` are already logged and skips them). Runs inside a 45s time budget (leaving margin under the file's 60s `maxDuration`), pacing submissions ~230ms apart (LeadSquared's own documented ceiling for this specific Async endpoint is 25 calls/5sec, i.e. one call per 200ms), logging each accepted row as `status:'pending'` with its real `request_id`. If the whole row list isn't drained inside the time budget, the function **fires an authenticated self-continuation request to itself** (`x-cron-secret` header matched against `CRON_SECRET`, same shared-secret pattern the existing GitHub Actions crons already use) carrying the remaining rows and the same `batchId` -- so a bulk import of any size keeps draining server-side, chained call after call, with zero dependency on the browser tab, the user's screen, or any external scheduler surviving.
- `opportunity_async_check_results` -- polls the Status API for up to 200 `status:'pending'` rows (optionally scoped to one `batchId`) and flips each to `success`/`failed` based on the real response.
- Both `listOpportunityBatches()` and `listOpportunityActivity()`'s truncated-count queries gained a 5th/4th parallel count for `pending`, so the History tab's aggregate numbers stay accurate without needing to page through rows.
- A self-continuation auth bypass was added at the top of `handler()` (same style as the existing `careers_sync`/`apps_sync` bypasses), gated on the exact `source`/`mode`/`x-cron-secret` combination this feature uses.

**Supabase (`supabase/sql/leadsquared_opportunity_activity_setup.sql`)**: appended `ALTER TABLE ... ADD COLUMN IF NOT EXISTS request_id TEXT` + an index -- `status` already had no CHECK constraint, so the new `'pending'` value needed no schema change beyond this column. Safe to re-run.

**Frontend (`src/pages/LeadSquaredDashboard.jsx`)**: `BulkOpportunityImport`'s `start()` now branches -- the direct-by-Opportunity-ID path (`isDirect`, which bulk import is locked to per the 2026-08-31 entry above) calls the new `submitAsync()` instead of the old browser-driven loop; the older Capture-Opportunities lead-matching path (`!isDirect`, only ever reachable from Single mode, never Bulk) is completely untouched. Added a "Resume this batch" affordance if a submission is interrupted, and `OpportunityHistoryTab` gained a "Pending" KPI tile (shown only when `stats.pending > 0`) plus a "Check results" button that calls the new status-poll mode.

**A real, unrelated hazard surfaced mid-session, not caused by this feature but worth recording**: this exact repo/working-directory is shared with at least one other concurrent Claude Code session (confirmed via `git log` showing an unrelated `feat(pmreport)` commit land mid-task, `dac4337`), which committed and pushed while most of this feature's edits were sitting uncommitted -- sweeping them into that unrelated commit. Resolved by NOT rewriting already-pushed history: re-verified the combined state still builds clean (`npm run build` + `node --check api/crm-leads.js`), then committed the small remainder as `eff2cec` with a full, accurate description of the whole feature. **Anyone touching `api/crm-leads.js` or `LeadSquaredDashboard.jsx` should assume a peer session may be editing the same files concurrently in this exact directory** -- commit early and often rather than batching a large diff.

**Live-verified end-to-end, to the extent possible without real Async API credentials configured**: pasted a 2-row test CSV via Bulk import > Opportunity ID (direct update), mapped the column, confirmed "2 ready to send", clicked submit (recovered from a native `window.confirm()` dialog blocking the browser-automation tool by pressing Return to accept it) -- the flow correctly ran parse -> map -> confirm -> POST to the new backend mode -> server-side credential guard -> a clean, actionable "Could not submit -- LeadSquared Async API is not configured -- enable it in LeadSquared (My Profile > Settings > API and Webhooks > Async API), then set LEADSQUARED_ASYNC_HOST and LEADSQUARED_ASYNC_API_KEY in Vercel env" message with a "Start a new import" recovery button. Zero crashes, zero real LeadSquared API calls attempted (the credential guard is the first thing checked in both new dispatch modes). Separately confirmed the History tab still renders correctly with the new pending-aware counting logic folded in -- real aggregate numbers (8,847 total attempts / 8,841 success / 0 duplicate / 6 failed / 8 bulk batches), no Pending tile shown (correct, since pending is genuinely 0 -- no row has ever reached the async-submit path yet), zero console errors.

**Requires user action before this is actually live** (Async API credentials don't exist yet, confirmed live above): (1) enable Async API in LeadSquared -- My Profile > Settings > API and Webhooks > Async API; (2) take the resulting async host + `x-api-key` LeadSquared gives back and set `LEADSQUARED_ASYNC_HOST` and `LEADSQUARED_ASYNC_API_KEY` in Vercel env (Production + Preview); (3) re-run `supabase/sql/leadsquared_opportunity_activity_setup.sql` in the Supabase SQL editor to add the new `request_id` column (safe to re-run, additive only). Until step 1-2 are done, every bulk-async submit attempt will cleanly fail with the exact "not configured" message confirmed above -- no silent corruption, no crash.

## 2026-09-03 (later) -- LeadSquared History tab: two stacked spinners fixed, then a real production crash found and fixed (commits `30017e0`, `19872ff`)

User flagged the History tab's loading state as messy -- two independently-positioned spinners ("Loading batches" then "Loading history") stacked with a gap between them, plus a flash of wrong "0/0/0/0" stats while both were still in flight. Also asked, short-answer, whether LeadSquared charges extra for the Async API used in the entry above -- checked LeadSquared's own docs directly: Async API is a plain "Enable: Yes" toggle in My Profile > Settings > API and Webhooks, no plan tier or add-on mentioned anywhere in the setup flow.

**First attempt (`30017e0`) crashed the page in production.** `listOpportunityActivity()`/`listOpportunityBatches()` fetch in parallel from `OpportunityHistoryTab`, resolving at slightly different times -- the fix added a single `rowsSettled`/`batchesSettled` gate and an early `return` showing one shared `InlineLoader` until both settle. That early return was placed BEFORE three `useMemo` calls further down the component (`stats`/`grouped`/`rowsByBatchKey`). React calls a different number of hooks on the loading render (skips all three `useMemo`s) than on the settled render (calls all of them) -- a genuine Rules-of-Hooks violation. Deployed, then caught live by immediately re-navigating to the page and clicking History: the tab crashed outright with `Minified React error #310` (rendered more hooks than the previous render) the instant both fetches resolved, replacing the whole tab with Quantum's generic "Something went wrong" error boundary.

**Fixed (`19872ff`)** by moving the loading check to a plain early `return` placed AFTER every hook in the component runs unconditionally on every render -- all `useState`/`useEffect`/`useCallback`/`useMemo` calls stayed exactly where they always were; only the JSX branching point moved. This is legal (React only requires the same hooks fire in the same order on every render *up to* wherever the component returns; an early return placed after every hook has already run doesn't skip any of them).

**Live-verified after the real fix, twice**: a fresh tab navigation showed exactly one spinner ("Loading Create Opportunity history"), settling into the correct real content (8,847 total attempts / 8,841 success / 0 duplicate / 6 failed / 8 bulk batches, full "All bulk imports" list, history rows below) with zero console errors; then switched away to Single and back to History to force a full remount (the tab unmounts/resets `rows`/`batches` to null each time, per its existing `mode === 'history' ? <OpportunityHistoryTab/> : (...)` conditional-render pattern) -- same clean single spinner, same correct settled content, zero errors, confirming the fix holds across remounts, not just first paint.

**A pre-existing, unrelated duplicate paragraph found and removed while updating this file**: this exact section of CLAUDE.md had a stray duplicate of the 2026-09-02 RankedBars entry's closing "Live-verified after deploy" paragraph sitting at the true end of the file, out of chronological order, after the entire 2026-09-03 entry above -- almost certainly a leftover of the concurrent-session collision documented in that same entry (`dac4337` sweeping up uncommitted edits from two sessions at once). Removed the duplicate; the original, correctly-placed copy right after the RankedBars entry is untouched.

**Lesson worth keeping**: a fix that only touches loading-state JSX can still violate the Rules of Hooks if it introduces a NEW early return anywhere in a component that also has hooks below that point -- always grep every hook call in the function and confirm the new return sits after all of them, not just visually inspect the diff. `npm run build` passed clean on the broken version too; this class of bug only surfaces at runtime, once the specific state transition (both fetches settling) actually occurs -- which is why it has to be caught by actually clicking through the live page after deploy, not by the build alone.

## 2026-09-04 -- Super Tracker: tiered Send-to-Slack channel picker (commit `9818b5d`)

Came out of a design pass done partly in the Chrome-extension surface, which correctly identified it has no filesystem/git access and handed back a design + a code sketch for the terminal session to implement (a real, useful division of labour worth remembering -- the extension is good for live inspection/design proposals, the terminal session for the actual file/build/push loop).

**What changed:** `src/components/SlackReportPanel.jsx` (shared by Overall, CEO B2C, Leverage Careers, Super Tracker) previously showed the same three flat chip rows on every page -- Sandbox / Real channels / Any channel the bot is in -- with a locked channel distinguished only by a ~10px lock glyph. Super Tracker now gets a genuinely different treatment, scoped via `dashboardId === 'super_tracker'` inside the existing render tree (a ternary at the exact spot the three old `<div>` groups used to render unconditionally): a **new `SuperTrackerChannelPicker` component** (added after the main export, alongside `TablePreview`/`FieldGrid`/etc) renders a two-tab tier switch -- Sandbox (cyan) vs Real channels (navy) -- each channel as a card (not a chip) with a clear selected state (tinted background + green checkmark), guarded channels carrying a persistent `LOCKED · PIN` badge and navy border regardless of selection so they read as gated before they're even clicked, and "any channel the bot is in" promoted out of the hidden `Browse channels` link into an always-visible search box under the Real tier (fires `loadChannels()` once on first switch to that tier via a small `useEffect`).

Every other dashboard's ternary branch is the byte-for-byte original three-group JSX -- confirmed unchanged live on `/dashboard/overall` after deploy. The shared logic underneath the picker (the bottom "reads" info line, the guarded phrase+PIN gate box, `send()`, `armed`/`gateErr`/`pinInfo` state) was not touched at all -- the new component only replaces the picker rows themselves and reuses the exact same `target`/`setTarget`/`DEST()`/`rawDest`/`channelSearch` state the original three groups already used, so there's no risk of the new UI and the old send-flow logic disagreeing about what's selected.

**Real-world friction hit during verification, unrelated to the change:** Super Tracker's own page (`SuperTrackerDashboard.jsx`, fetching `/api/crm-leads?source=super_tracker`) took ~30-40s to resolve its first load on a cold hit -- looked exactly like a stuck/broken page (persistent branded spinner, `readyState:'complete'`, zero console errors, zero relevant network requests initially captured) until a second check caught the request actually landing 200 a few checks later. Confirmed this was pre-existing and unrelated to this change by loading `/dashboard/overall` in the same tab immediately after and seeing it resolve in ~2s. Worth remembering: a page that fetches directly from an external source (Google Sheet/BigQuery-backed) on this codebase can genuinely take 30-100+ seconds on a cold load (documented elsewhere in this file for several other pages) -- don't assume a change broke a page just because it's still spinning after 10-15s; verify with an unrelated page first, and check network requests for a real pending/resolved call before concluding it's stuck.

**Live-verified end-to-end** on the real production site (admin session): Super Tracker's Send to Slack panel renders the new tier switch; Sandbox tab shows `#voxpath`/`#dashboard-testing` as plain cards with the cyan-active tab and "Test sends -- only your team's sandbox channels see these." subtitle; Real channels tab shows `team channel` (plain) plus `CEO channel`/`B2C core channel` each with the lock icon + navy `LOCKED · PIN` badge, and an always-visible "OR SEARCH ANY CHANNEL THE BOT IS IN" input (no click needed to reveal it); clicking the CEO channel card selected it correctly (navy border, tinted background, green checkmark) and triggered the existing, untouched guarded-gate box below ("The CEO is in this channel. Admin, phrase and PIN.", the phrase input pre-filled with the placeholder `SEND TO #performance_mktg_core`, and the PIN input) -- confirming the new picker and the old shared send-flow logic talk to each other correctly through the same `target` state. Re-checked `/dashboard/overall` afterward and confirmed its picker is completely unchanged (still the three flat chip-row groups, no tier switch, no cards). Zero console errors on either page.
