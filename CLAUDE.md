...
## 2026-09-23 -- Overall BigQuery beta opened to Sri Teja; Apps page/Slack report blank-and-zero incident root-caused, data restored, prune bug fixed (commits `4f76cbd`, `4600ba6`)

Two unrelated support tickets, same session. (1) "Sri Teja can't access Overall (BigQuery)" -- root cause was the hardcoded `OVERALL_BIGQUERY_EMAILS` allowlist in `shared/access.mjs`, which gates that one page BEFORE the normal role/grant logic and short-circuits it entirely -- an admin or a user with every other page granted can still be fully blocked if their email isn't on this specific list. Found his real email via Settings > User Access search (not guessed), added `chilukoti.sriteja@leverageedu.com` to the array. Since `lib/auth.mjs`, `src/App.jsx`, and `src/components/Sidebar.jsx` all import `canAccessDashboard` from this one file, the fix propagated to the route guard, sidebar nav, and server-side API gate simultaneously. Live-verified via a content-check on the deployed chunk and a page load as the admin session; could not verify from Sri Teja's own login (no credentials) -- asked him to confirm.

(2) "Apps page blank, Slack Marketing Efficiency report shows 0 Applications -- check asap." Root-caused via `bigquery_jobs` history: the 2026-09-23 04:01 UTC (9:31 AM IST) scheduled `apps_sync` run processed 0 bytes/0 rows (every other day for the prior week processed ~36.5GB/~7,300 rows) -- a real, transient upstream BigQuery availability blip, not a Quantum bug. But `api/crm-leads.js`'s `apps_sync` handler ran its own prune step (`DELETE apps_feed WHERE sync_id != <this run's id>`) **unconditionally**, even though the upsert loop had zero rows to write that run -- so every pre-existing row got deleted since none carried the fresh sync_id, wiping the table to 0. `.github/workflows/apps-sync.yml`'s own trigger only checked HTTP status (always 200 here), never the response body's `rowCount`, so the workflow reported a green "success" while quietly destroying the data. Exactly the same bug class as the 2026-09-17 Overall BQ sync agg-table incident.

**Data restored first, same session**: manually re-triggered the sync via an authenticated admin browser session (working around the CRON_SECRET gate by falling through to normal admin-session auth), confirmed via a direct Supabase query that a fresh, successful sync landed (7,357 rows, 36.5GB scanned, matching the historical daily pattern exactly), and confirmed live that `/dashboard/apps` rendered fully correct data again (Total Applications 7,357, Distinct Students 7,355, full breakdowns, "Synced 23/09/2026, 11:08:59"). The Slack report reads the same now-restored table, so its 0-Applications figure is fixed by the same restoration.

**Then the underlying bug fixed** (`4600ba6`): `apps_sync`'s prune line now only runs when `rows.length > 0` -- a 0-row BigQuery result now leaves yesterday's good data in place instead of wiping it, mirroring the 2026-09-17 fix's guard pattern exactly. `.github/workflows/apps-sync.yml` now also parses the JSON body (`jq`) and posts a GitHub Actions `::warning::` if `rowCount` comes back 0, so a future transient hiccup is visibly flagged instead of silently reported as a clean success -- it no longer fails the workflow on 0 rows (since that case is no longer destructive), only on a genuine `ok !== true`. `node --check` + `npm run build` clean; `git diff --cached --stat` confirmed only the 2 intended files staged before commit.

**Not yet independently re-verified**: the next scheduled `apps-sync.yml` run (9:30 AM IST) hasn't fired since this fix shipped -- worth a quick check tomorrow that a normal run still prunes correctly (non-zero rowCount, prune still happens, table doesn't silently grow stale rows) and that the new `::warning::` path only fires on a genuine 0-row response, not on every run.

## 2026-09-23 (later) -- Deep-dive on GitHub Actions failures: 3 real, distinct recurring bugs found and fixed (commits `8815d96`, `99a67e5`)

User: "one of the actions get failed on a daily basis... I would like to know why and I just want to remove this failure permanently." Pulled ~300 runs of real workflow history via the GitHub Actions API (public, no auth needed) across every scheduled workflow, then read the actual job logs (via an authenticated Chrome session -- job logs need write access even on a public repo, confirmed via a 403 on the plain REST API) for every failure, rather than guessing from symptoms. Three separate, real, recurring bugs, no fabricated pattern-matching -- each confirmed against multiple independent real failures before being called a "root cause."

**1. Overall BigQuery Cache Sync -- the one felt as "daily."** Failed at 04:30 UTC (10:00 AM IST) on 7 of the last ~11 days (09-12, 09-15, 09-17, 09-20, 09-21, 09-22, 09-23), always with the identical traceback at the same script line. Root cause: the post-sync "exact row count" verification query (a plain `count=exact` PostgREST call, pure logging/sanity-check, zero side effects) had no try/except. It hit the same real Postgres `57014` statement-timeout the prune step right above it already tolerates gracefully -- but being unguarded, it crashed the entire script with exit code 1 and silently skipped the whole agg-table sync every time, even though the real raw-table data had already landed correctly. This is exactly why the Overall dashboard's fast path / MTD Scorecard Slack report kept looking stale despite the raw sync "succeeding." Fixed (`8815d96`): wrapped both the raw-table and agg-table row-count checks (`overall-bq-sync.yml`) in the same try/except resilience the prune step already has -- a genuine mismatch still fails the run; a timeout on the check itself now just logs a warning. **Live-verified, not just pushed**: manually triggered a real run afterward (#300) -- succeeded, and a direct Supabase query confirmed `overall_bq_daily` and `overall_bq_daily_agg` now share the identical fresh `synced_at`, proving the agg table actually got rebuilt this time (previously it silently never did).

**2. Leverage Careers BigQuery Cache Sync -- failed 3x in one week** (09-12, 09-13, 09-14), always the identical `Supabase upsert failed: {"message":"Gateway Timeout"}` on one random batch (batchIndex 2, 2, 4 of ~41). The upsert loop was a plain sequential `for` with a single POST attempt per batch and an immediate `return res.status(502)` on the first failure -- no retry at all. This exact codebase already has a proven retry-with-backoff helper, `upsertBatchesConcurrent` (`api/crm-leads.js`, defined for `overall_bq_sync`'s agg-table upsert), just never wired up here. Fixed: switched `careers_sync`'s loop to call it (batchSize 500, concurrency 8, matching the only other caller) -- a transient timeout on one batch now gets retried up to 3x with backoff before the run is allowed to fail for real; prune still only runs after every batch has genuinely landed. **Live-verified**: manually triggered run #94 on the fix commit -- succeeded cleanly.

**3. Quantum Gazette Agent -- crashed twice** (09-20, 09-21) with a genuine server error, `Cannot read properties of undefined (reading 'map')` -- not a timeout, a real code bug. Root-caused via a dedicated Explore-agent trace through the whole `agent_run`/`quantum_gazette` code path (`api/ask-ai.mjs` -> `buildQuantumGazetteEdition` -> `shared/gazetteTemplate.mjs`'s `editorsNote()`): the ONE Anthropic call that writes the Gazette's headline/lede/editor's-notes (`generateGazetteProse`) was capped at `max_tokens: 3072` -- tight enough, across a headline+lede+pull-quote+byline+3 full sections (7 editor's-note items total) in one structured-output response with no follow-up round to recover in, that the model can finish a section's `intro` while never emitting its `notes` array at all under truncation. `editorsNote(title, subtitle, items)` then called `items.map(...)` with zero guard, crashing the whole agent run. This exact file already has a documented precedent for this exact failure mode (2026-08-24, a sibling tool loop bumped 1024 -> 4096 after an identical silent-truncation incident) -- `generateGazetteProse`'s own call had just never gotten the same fix. **Fixed two layers, root cause + defensive net**: bumped `max_tokens` 3072 -> 4096 to match the sibling call's already-proven-sufficient budget; hardened `editorsNote()` to `Array.isArray(items) ? items : []` plus optional-chained the 3 call sites (`prose.sectionA?.notes` etc.) so a still-malformed future response degrades to an empty section instead of taking the whole run down. **Deliberately not live-fired**: triggering this manually would cost a real Anthropic API call, overwrite the live-facing `gazette_html` mid-day, and -- since the bug is a probabilistic truncation risk, not deterministic -- a single success wouldn't have been conclusive proof either way. Left to verify itself on tonight's real 9:30 PM IST scheduled run; the defensive guard means even an unlucky truncation can no longer crash the run regardless.

**What was checked and correctly NOT touched**: Report Scheduler (Email), Overall Funnel Cache Sync, Apps BigQuery Cache Sync, and Monthly QLs BigQuery Cache Sync all showed failures too, but every single one of them landed on 2026-09-19 -- the already-documented GitHub Actions billing/budget incident (the repo had quietly gone private, hit its Actions-minutes allowance, and was made public again that same day). Confirmed via the full run history that none of these four have failed on any other day -- a real, already-resolved one-time infra event, not a current recurring bug, so no code change was made for any of them.

`node --check` clean on all touched files (`overall-bq-sync.yml`'s embedded Python via `ast.parse`, `api/crm-leads.js`, `api/ask-ai.mjs`, `shared/gazetteTemplate.mjs`); `npm run build` clean; `git diff --cached --stat` confirmed only the intended files staged before each commit.

## 2026-09-24 -- Overall: TOTAL QLs KPI card opens Daily QLs, embedded (commit `dfd7fdc`)

User: "total qls on overdashboard should open up daily qls page, we did it for apps and etc" -- direct follow-up to the already-shipped Applications/Human QL/AI QL drill-downs. This card was previously deliberately left unwired (per its own code comment, 2026-09-07): Total QLs sums three channels (Human + Futwork AI + Superbot) and no single detail page covered all three. Daily QLs (`LeadQualificationDashboard.jsx`, `forcedView="daily"`) genuinely does show all three together, so it's the honest drill target now that it's being wired up.

**Shipped**: `LeadQualificationDashboard.jsx` gained `{embedded, initialFrom, initialTo}` props, the exact same shape `AppsDashboard.jsx`/`HumanQLDetailDashboard.jsx`/`AIQLDetailDashboard.jsx` already have -- `embedded` skips the Sidebar + full-page 100vh shell, `initialFrom`/`initialTo` (YYYY-MM-DD) seed `datePreset='custom'` + `customFrom`/`customTo` instead of the page's own default current-month view. Unlike the other three embeddable pages, this one had no prior early-return loading skeleton to build the split around -- its loading state was always handled inline within one single `return (...)` -- so this needed restructuring the whole return into `const content = (<>...</>); if (embedded) return content; return (<div className="lq-page-shell"><Sidebar/>{content}</div>)`, dropping the outer shell/Sidebar and making the card wrapper's margin/boxShadow/flex/overflow conditional on `embedded`, matching `HumanQLDetailDashboard.jsx`'s own established pattern line-for-line.

`OverallDashboard.jsx`: `qlDrillOpen` widened to accept `'qls'` alongside `'human'|'ai'|'apps'`; a new `DailyQLsEmbed = React.lazy(() => import('./LeadQualificationDashboard'))` (shares the same chunk `App.jsx` already lazy-loads for the real `/dashboard/lq-ops` route -- no bundle duplication); the TOTAL QLs `HighlightKPI` card wrapped in the same `lq-kpi-clickable`/`onClickCapture`/`onKeyDown` treatment as Human/AI QLs (capture-phase, not bubble, for the same `kpiVariants.jsx` `stopPropagation()` reason already documented on those two cards); the drill-down modal gained a `'qls'` title (`"Daily QLs"`) and subtitle branch stating the three-channel combination explicitly. The stale "deliberately NOT wired" code comment (which explained why Total QLs had no drill-down) was rewritten to describe the new behavior instead of contradicting it.

**Live-verified end-to-end** on quantum.leverageedu.com: clicked the real TOTAL QLs card (via a dispatched click on the `.lq-kpi-clickable` wrapper, confirmed by its `title` attribute) -- modal opened titled "Daily QLs" with subtitle "Human + Futwork AI + Superbot combined, the real per-lead records behind that number, 2026-09-01 to 2026-09-30" (Overall's active month was Sep'26, and `activeDrillDownRange` correctly resolved to that month's real start/end). The embedded page took ~25s to load on this genuinely cold fetch (`DashboardSkeleton`'s spinner has no text, which reads exactly like a stuck/empty state until you wait it out -- same false-alarm pattern this file's own 2026-09-07 entry already documented for the Human/AI QL drill-down's first verification pass; waited it out rather than assuming a bug) then rendered full real data: Total Qualified 7,987 (Futwork 4,845 / Futwork AI 3,142 / Superbot 0), SR 4,291 / AC 3,696, Top Country UK, real charts, "Synced 14:09:45" -- a real, close-but-not-identical figure to Overall's own TOTAL QLs KPI (8,094) for the same nominal window, expected since these are two independently-computed pipelines (same disclosed-divergence pattern this app already has for the Apps drill-down). Clicked "Last 7D" inside the modal -- Total Qualified correctly changed to 2,274, confirming the embedded controls are genuinely live, not a frozen snapshot. Zero real console errors. `npm run build` clean; `git diff --cached --stat` confirmed only the 2 intended files staged.

## 2026-09-25 -- "Attempted, Not Closed": WHY real staff appear as Opportunity Owner -- root-caused, not yet fixed (per explicit instruction)

Direct continuation of the 2026-09-25(-dated in-session, logged here on push) investigation into 22 real human names appearing as Opportunity Owner on 25 "Attempted, Not Closed" rows this month, all Human-channel, all genuine terminal non-QL dispositions. User's explicit instruction both times: "check it, but don't initiate anything" / "why is this happening, don't fix yet" -- this entry is the read-only answer, no fix applied.

**Investigated using LeadSquared's own real field-change audit log, not a diagnostic mode.** `api/crm-leads.js` already exposes `mode=activities&leadId=<id>` (`fetchLeadSquaredActivities`, `/v2/ProspectActivity.svc/Retrieve`) -- the SAME endpoint the Field Schema page already uses -- which returns a lead's FULL activity timeline across every event type, including LeadSquared's own auto-logged "University Admission Opportunity" field-change records (`Data: [{Key:'Event',Value:'Update'}, {Key:'OldData',...}, {Key:'NewData',...}, {Key:'CreatedByName',...}]`). This carries a genuine before/after diff plus the real actor for every field edit -- exactly what's needed, and something the activity-search-by-disposition endpoint (tried first, in a now-removed temporary diagnostic mode `owner_investigation_debug`) could never answer, since `CreatedByName`/`Owner` come back null on that endpoint's own Activity records (Owner lives on the Opportunity, not the Activity). Cross-referenced actor/owner GUIDs against the existing `mode=team_users` roster to resolve real names.

**Two distinct, both-genuine, both-confirmed-via-real-data mechanisms, not one:**

1. **Manual staff override.** Sample: `eed595fc...` (Radhika Aggarwal, one of the original 25). Her own audit log shows Owner changed from `Futwork` -> `Radhika Aggarwal` at 2026-09-25 09:56:35, actor `Vaishnavi Dubey` (a real staff member, not a bot), with `mx_Custom_75` ("Last Status from Futwork") flipped from `"No Action"` to `"De-Queued"` in the SAME edit. A real person, inside LeadSquared's own UI, manually pulled this Opportunity out of the Futwork queue and reassigned it directly to a floor counselor.

2. **An existing, automated LeadSquared assignment rule, firing off a DIFFERENT field than the one Quantum classifies by.** Sample: `6447e516...` (owner Md Hasnain Ali Ansari). Its audit log shows Owner auto-changed from `Futwork` -> `Md Hasnain Ali Ansari` by `System Test` (LeadSquared's own automation identity, not a person) the instant `mx_Custom_86` -- confirmed via `GetOpportunityTypeMetadata` to be literally named **"Distribute Opportunity"** -- was set to `"Yes- Post Call Futwork Qualified"`. Critically, **`mx_Custom_86` ("Distribute Opportunity") is a different field from `mx_Custom_100` ("Last Disposition from Futwork")** -- the one field the Not Attempted/Attempted Not Closed page's own classification logic actually reads. So a lead can trip this "Distribute Opportunity = Yes" signal and get handed to a real counselor, while `mx_Custom_100` independently still reads (or later gets overwritten to) a terminal non-QL value like "Not Connected" -- landing the same Opportunity in "Attempted, Not Closed" with a real human Owner, with nothing wrong in either field individually.

**Net conclusion, stated for the record**: neither mechanism is a Quantum bug. The page correctly reads `mx_Custom_100`/`mx_Custom_58` for classification, exactly as designed. Owner is a separately-managed LeadSquared field that can diverge from disposition state via (a) a real staff member's own manual queue-pull/reassignment, or (b) LeadSquared's own pre-existing "Distribute Opportunity" auto-routing rule, which is not synced to the disposition field this page classifies by. Both are real, ongoing, and expected to keep producing a small trickle of "real owner, non-QL disposition" rows -- not a one-time event.

**No fix applied, per explicit instruction.** If a fix is ever wanted, the two live options (not yet scoped or asked about) are: (a) leave Owner display as-is and simply stop treating "who owns it" as a proxy for "is this genuinely still with Futwork" anywhere in the UI/reporting, since it demonstrably isn't reliable for that; or (b) add a distinct "currently still queued with Futwork" indicator sourced from `mx_Custom_75` ("Last Status from Futwork") instead of Owner, since that field's own value (`Queued on Futwork` vs `De-Queued` vs `Queue Failed`) is the one LeadSquared field that actually tracks this directly.

Diagnostic cleanup: the temporary `owner_investigation_debug` mode added earlier this session (never actually useful -- its endpoint returns null for `Owner`/`CreatedByName` on Activity records) was removed from `api/crm-leads.js` and `NOT_ATTEMPTED_MODES`; `git diff` against the pre-diagnostic commit confirmed a clean, net-zero removal. `node --check` clean. Pushed straight to `main` (`1e649fc`) -- this cleanup commit contains no functional change, only the removal.
