...
async function handleLeadSquared(req, res, me) {
  ...
  try {
    ...
    // disposition (Human: mx_Custom_100, AI: mx_Custom_58) is a non-QL value. Step 1
    // (headOnly=1) is a fast count-only probe to size the real dataset before paginating.
    // Remove after use.
    if (mode === 'live_ql_open_nonql_debug') {
      const headOnly = req.query.headOnly === '1'
      ...
      return res.status(200).json({ totalOpenFetched: rows.length, openNonQlTouched, dispCounts })
    }
