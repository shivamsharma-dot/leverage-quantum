...
export default async function handler(req, res) {
  ...
  // TEMPORARY diagnostic-only branch, added 2026-09-14 to live-test the real
  // Super Tracker v3 builder end-to-end (multi-block bundled messages, not
  // just isolated block types) in the #voxpath sandbox before calling it
  // done. Remove this whole branch once the live test is done.
  if ((req.body?.type || req.query?.type) === 'super_tracker_v3_diag_test' && req.query?.key === 'st3_diag_2026_0914') {
    ...
    const section1 = {
      ...
      rows: [
        ...
        mkRow('R02', 'Revenue', 'AC Sales', 'B2C', 'Aman', 'Yes', ['18', '21', '']),
      ],
    }
    const section2 = {
