...
// 'upskilling' (revenue) and 'corpSalary' (cost) are two columns Finance added
// to the Daily P&L tab in 2026-09 (api/crm-leads.js's B2C_PNL_COLS has the
// full story) -- P&L-only, so REV_CASHFLOW is untouched; Cash Flow rows carry
// no such fields, so col(rows,...) below just resolves them to null there,
// same as ebitdaBeforeCorp already does.
const REV_PNL = [['srOnline'], ['ac'], ['vas'], ['srOffline'], ['acOffline'], ['vasOffline'], ['upskilling']]
