...
export async function fetchYoutubeAnalyticsRange(creds, since, until) {
  const token = await youtubeAccessToken(creds)
  // Explicit channel id, not 'channel==MINE' -- MINE resolves to whatever
  // channel is the OAuth-granting account's own default context, which is
  // not guaranteed to be this specific (possibly Brand Account / delegated)
  // channel. Confirmed live: MINE returned all-zero rows even for a fully
  // settled past month on a channel with 38M+ lifetime views.
  const ids = `channel==${creds.channelId}`
