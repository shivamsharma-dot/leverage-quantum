...
// TEMPORARY diagnostic helper -- returns the first ~30 raw rows of one named
// tab, unparsed, so an unrecognized tab's real content can be inspected
// before deciding how (or whether) to parse it. Remove once no longer needed.
export async function fetchSuperTrackerRawTab(spreadsheetId = DEFAULT_SHEET_ID, tabTitle) {
  const token = await accessToken(superTrackerCreds())
  const range = encodeURIComponent(`'${tabTitle}'!A1:Z30`)
