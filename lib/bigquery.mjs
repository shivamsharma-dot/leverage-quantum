...
// bigQuerySelect caps at 20,000 rows in ONE call, silently -- BigQuery's own
// jobs.query response is itself paginated, and a result wider than that comes
// back truncated with no error at all. Fine for the interactive Settings
// Console and a narrow careers_leads window (both comfortably under the cap),
// wrong for anything whose true row count can grow past it. This IS exactly
// what happened the first time leverage_careers_daily was synced: the real
// result was 19,677 rows, bigQuerySelect (no maxResults passed, so it
// defaulted to 1,000) handed back only the first 1,000, and the cache was
// silently 95% empty with no error anywhere. This variant follows BigQuery's
// own pageToken until every row is collected, and refuses to return a short
// read rather than trusting whatever the first page happened to contain.
export async function bigQuerySelectAll(sql, opts = {}) {
  const creds = bigQueryCreds()
  const token = await bigQueryAccessToken(creds)
  const location = opts.location || creds.location
  const payload = {
    query: assertReadOnly(sql),
    useLegacySql: false,
    location,
    timeoutMs: Math.min(Number(opts.timeoutMs) || 45000, 55000),
    maxResults: 20000, // the most one page can hold; pageToken covers the rest
  }
  payload.maximumBytesBilled = String(bytesBilledCap(opts.maxBytes))
  const t0 = Date.now()
  const baseUrl = BQ_HOST + '/projects/' + encodeURIComponent(creds.projectId) + '/queries'
  ...
  const jobId = (d.jobReference && d.jobReference.jobId) || null
  const jobLocation = (d.jobReference && d.jobReference.location) || location
  let rawRows = Array.isArray(d.rows) ? d.rows.slice() : []
  let pageToken = d.pageToken
  let pages = 1
  while (pageToken) {
    if (pages > 500) throw new Error('BigQuery: more than 500 result pages -- refusing to loop forever.')
    const pageUrl = BQ_HOST + '/projects/' + encodeURIComponent(creds.projectId)
      + '/queries/' + encodeURIComponent(jobId)
      + '?location=' + encodeURIComponent(jobLocation)
      + '&maxResults=20000&timeoutMs=55000&pageToken=' + encodeURIComponent(pageToken)
    const pr = await fetch(pageUrl, { headers: { Authorization: 'Bearer ' + token } })
    const pd = await pr.json().catch(() => ({}))
    if (!pr.ok) {
      const e = (pd && pd.error) || {}
      throw new Error('BigQuery (page ' + (pages + 1) + '): ' + (e.message || ('HTTP ' + pr.status)))
    }
    if (Array.isArray(pd.rows)) rawRows = rawRows.concat(pd.rows)
    pageToken = pd.pageToken
    pages += 1
  }
  const totalRows = Number(d.totalRows || 0)
  if (totalRows && rawRows.length !== totalRows) {
    throw new Error('BigQuery pagination drop: collected ' + rawRows.length + ' rows over ' + pages
      + ' page(s) but the job reported totalRows=' + totalRows + '. Refusing to return a short read.')
  }
  const out = {
    rows: decodeRows(d.schema, rawRows),
    fields: ((d.schema && d.schema.fields) || []).map(f => ({ name: f.name, type: f.type })),
    totalRows,
    totalBytesProcessed: Number(d.totalBytesProcessed || 0),
    cacheHit: Boolean(d.cacheHit),
