    ...
    items: [
      {
        ...
        subItems: [
          ...
          { to: '/dashboard/roas', label: 'ROAS', matchType: 'route' },
          { to: '/dashboard/mtd', label: 'MTD', matchType: 'route' },
        ]
      },
...
export default function Sidebar() {
  ...
  const isMetaParentActive = location.pathname.startsWith('/dashboard/meta-ads') ||
    location.pathname === '/dashboard/roas' ||
    location.pathname === '/dashboard/mtd'
            ...
            {group.items.filter(item => canSee(idMap[item.label])).map(item => {
              if (item.subItems) {
                return (
                  <div key={item.label}>
                    <button
                      className={`${styles.navItem} ${isMetaParentActive ? styles.active : ''}`}
                      onClick={() => { setMetaExpanded(e => !e); if (!isMetaParentActive) navigate(item.to + '?tab=campaigns') }}
                      ...
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"
                        style={{transform: metaExpanded ? 'rotate(180deg)' : 'none', transition:'transform .2s', opacity:.4}}>
