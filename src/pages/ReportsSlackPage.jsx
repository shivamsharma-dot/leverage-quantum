...
// Settings > Reports redesign (2026-08-27): Slack split out into its own real,
// bookmarkable route. Covers the team/test channel roster and the new
// "browse every real channel" picker (slack_channel_list, via
// conversations.list) that replaces free-typing a channel ID.
//
...
// (guarded channels, confirmPhrase, the PIN itself) and rushing that move
// risks breaking it silently. It still lives, working and untouched, on the
// old Reports tab in Settings for now; this page focuses on the plain
// channel roster and the new channel browser.

export default function ReportsSlackPage() {
  const [loading, setLoading] = useState(true)
  const [channelMain, setChannelMain] = useState('')
  const [testChannels, setTestChannels] = useState([])
  const [auto, setAuto] = useState(true)
  const [allChannels, setAllChannels] = useState(null) // null = not loaded yet
  const [channelsErr, setChannelsErr] = useState('')
  const [channelsLoading, setChannelsLoading] = useState(false)
  const [picker, setPicker] = useState(null) // 'main' | 'new' | null
      ...
      <div className={styles.main}>
        <div className={styles.header}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <span style={{ width: 30, height: 30, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><SlackIcon size={26} /></span>
            <h1 className={styles.title} style={{ background: 'none', WebkitTextFillColor: 'initial', color: 'var(--text)' }}>Slack</h1>
          </div>
          <p className={styles.titleSub}>Posts as @pm_analyst · CEO-locked channels are managed on the Reports tab in Settings for now</p>
        </div>
          ...
          {loading ? <p style={{ color: 'var(--text-3)', fontSize: 13 }}>Loading…</p> : (
            <>
              ...
              <p style={{ fontSize: 11.5, color: 'var(--text-3)', marginTop: 8 }}>
                CEO-locked channels, the PIN, and B2C approval routing are still on the old Reports tab (Settings &gt; Reports) while that security-sensitive flow gets migrated separately.
              </p>
            </>
          )}
        </div>
