...
// Channel hues from the shared map. The old one put Bing on off-brand amber and
// gave LinkedIn the same blue as Facebook.
const CH_COLORS = SOURCE_COLORS
...
export default function ROASDashboard(){
  ...
  const kpis=[
    ...
    {label:'ROAS',          value:tot.roas+'x',      prevRaw:prevTot?.roas,     raw:tot.roas,           sub:'Rev / Spend',  color:tot.roas>=1?'#4CAE6F':tot.roas>=0.5?'#1F3C84':'#9CA3AF'},
    ...
    {label:'QLs',           value:fn(tot.qls),       prev:prevTot?.qls,         sub:'Qualified Leads',  color:'#4CAE6F'},
    {label:'L→Q%',          value:tot.ltq+'%',       prev:null,                 sub:'Conversion Rate',  color:'#29B9C3'},
    {label:'Apps (STUs)',   value:fn(tot.stus),      prev:prevTot?.stus,        sub:'Uni Applications', color:'#4CAE6F'},
    {label:'CPL',           value:tot.cpl>0?fmt(tot.cpl):'–',prev:prevTot?.cpl,sub:'Cost per QL',      color:'#1F3C84'},
  ]
              ...
              <tbody>
                {(()=>{
                  ...
                  return Object.values(map).sort((a,b)=>b.spend-a.spend).map(r=>{
                    ...
                    const roasColor=parseFloat(roas)>=2?'#4CAE6F':parseFloat(roas)>=1?'#1F3C84':'#9CA3AF'
                    return(
