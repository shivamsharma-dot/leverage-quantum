...
const TS={contentStyle:{background:'#fff',border:'1px solid #E5E7EB',borderRadius:8,fontSize:12},labelStyle:{color:'#111827',fontWeight:600}}
...
export default function ROASDashboard(){
  ...
  // Spend trend — all 12 months
  const spendTrend=useMemo(()=>
    MONTHS.map(m=>{
      const rows=ROWS.filter(r=>r.month===m&&(selChannel==='All'||r.channel===selChannel))
      return{month:m.replace('-2025',''),spend:rows.reduce((s,r)=>s+r.spend,0)}
    }),[selChannel])

  // ROAS trend — only months with revenue
  const roasTrend=useMemo(()=>
    MONTHS.map(m=>{
      ...
      const roas=spend>0&&rev>0?+(rev/spend).toFixed(2):null
      const projRoas=spend>0&&projRev>0?+(projRev/spend).toFixed(2):null
      return{month:m.replace('-2025',''),roas,projRoas,rev,projRev}
    }).filter(d=>d.roas!==null||d.projRoas!==null)
  ,[selChannel])

  // Revenue trend — only months with revenue
  const revTrend=useMemo(()=>roasTrend,[roasTrend])
  ...
  const kpis=[
    ...
    {l:'Proj ROAS',    v:tot.proj_roas+'x', pv:prevTot?parseFloat(prevTot.proj_roas):null,rv:parseFloat(tot.proj_roas),s:'Proj/Spend',c:'kpi_bl'},
      ...
      <main className={styles.main}>
        ...
        <div className={styles.filters}>
          ...
          {prevMonth&&selMonth!=='All'&&(
            <div className={styles.momChip}>📊 MoM: {selMonth} vs {prevMonth}</div>
          )}
        </div>
        ...
        <div className={styles.chartsRow}>
          <div className={styles.chartCard}>
            <h3 className={styles.chartTitle}>Monthly Ad Spend — 2025</h3>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={spendTrend} margin={{top:4,right:8,left:0,bottom:0}}>
                <XAxis dataKey="month" tick={{fontSize:10,fill:'#9CA3AF'}}/>
                <YAxis tick={{fontSize:10,fill:'#9CA3AF'}} tickFormatter={v=>fmt(v)} width={70}/>
                <Tooltip {...TS} formatter={v=>[fmt(v),'Spend']}/>
                <Bar dataKey="spend" name="Spend" fill="#1C9FD4" radius={[4,4,0,0]}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
          <div className={styles.chartCard}>
            <h3 className={styles.chartTitle}>ROAS vs Projected ROAS — Aug to Dec</h3>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={roasTrend} margin={{top:4,right:16,left:0,bottom:0}}>
                <XAxis dataKey="month" tick={{fontSize:10,fill:'#9CA3AF'}}/>
                <YAxis tick={{fontSize:10,fill:'#9CA3AF'}} tickFormatter={v=>v+'x'} width={45}/>
                <Tooltip {...TS} formatter={(v,n)=>[v!=null?v+'x':'–',n]}/>
                <Legend wrapperStyle={{fontSize:11}}/>
                <Line type="monotone" dataKey="roas"     name="ROAS"      stroke="#4BAE8A" strokeWidth={2.5} dot={{r:5,fill:'#4BAE8A'}} activeDot={{r:6}}/>
                <Line type="monotone" dataKey="projRoas" name="Proj ROAS" stroke="#818CF8" strokeWidth={2} strokeDasharray="5 5" dot={{r:4,fill:'#818CF8'}}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className={styles.chartsRow}>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={channelBreakdown.filter(e=>e.spend>0)} cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={3} dataKey="spend">
                  {channelBreakdown.filter(e=>e.spend>0).map((e,i)=><Cell key={i} fill={CH_COLORS[e.channel]||'#9CA3AF'}/>)}
                </Pie>
                <Tooltip {...TS} formatter={v=>[fmt(v),'Spend']}/>
              </PieChart>
            </ResponsiveContainer>
          ...
          <div className={styles.chartCard}>
            <h3 className={styles.chartTitle}>Revenue vs Projected — Aug to Dec</h3>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={revTrend} margin={{top:4,right:8,left:0,bottom:0}}>
                <XAxis dataKey="month" tick={{fontSize:10,fill:'#9CA3AF'}}/>
                <YAxis tick={{fontSize:10,fill:'#9CA3AF'}} tickFormatter={v=>fmt(v)} width={75}/>
                <Tooltip {...TS} formatter={v=>[fmt(v)]}/>
                <Legend wrapperStyle={{fontSize:11}}/>
                <Bar dataKey="rev"     name="Actual Rev" fill="#4BAE8A" radius={[4,4,0,0]}/>
                <Bar dataKey="projRev" name="Proj Rev"   fill="#818CF8" radius={[4,4,0,0]} fillOpacity={0.7}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
