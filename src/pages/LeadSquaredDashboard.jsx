...
// Was LeadSquared's own raw yellow/green/red Status OptionSet colors, pixel-matched to
// its native Manage Opportunities screen -- but this app's own standing rule bars
// yellow/amber/red on any UI element (brand palette only), and every other status pill on
// this exact page (Create Opportunity's OPP_STATUS_STYLE below) already uses this same
// blue/green/navy vocabulary for the identical neutral/good/bad semantics. Aligned here
// too rather than mirroring LeadSquared's own raw pill colors.
const STATUS_COLORS = {
  Open: { bg: '#E8F6FA', fg: '#1C9FD4' },
  Won: { bg: '#E9F8EF', fg: '#178A54' },
  Lost: { bg: '#E8EFF9', fg: '#1F3C84' },
}
...
// Was a pair of native <input type="date"> -- against this app's own standing rule
// (custom-styled controls everywhere, never a native date/select input; DateRangePicker
// exists precisely for this). Same external API (since/until strings, onSince/onUntil
// setters) so none of the 3 call sites needed to change, only this component's internals.
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button type="button" onClick={() => setOpen(v => !v)} style={{
        ...
        border: '0.5px solid ' + (open ? '#1C9FD4' : C.border), background: open ? '#E8F6FA' : 'var(--card)', cursor: 'pointer',
      }}>
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" style={{ flexShrink: 0 }}><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>
        <span style={{ fontWeight: 800, fontSize: 10, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Created</span>
        <span style={{ whiteSpace: 'nowrap' }}>{since} &rarr; {until}</span>
      </button>
      {open && (
        <>
          ...
          <div style={{ position: 'absolute', left: 0, top: 'calc(100% + 8px)', zIndex: 400, background: 'var(--card)', border: '0.5px solid ' + C.border, borderRadius: 14, boxShadow: '0 20px 60px rgba(15,23,42,0.16), 0 4px 12px rgba(15,23,42,0.06)', overflow: 'hidden' }}>
            <DateRangePicker
              from={parseYmd(since)}
              to={parseYmd(until)}
              onChange={(f, t) => { if (f) onSince(f); if (t) onUntil(t); setOpen(false) }}
