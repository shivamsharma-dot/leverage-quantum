...
import { C, SOURCE_COLORS, BarGrad, barFill, BAR_RADIUS, BAR_MAX, NEUTRAL_TRACK } from '../ui/dashboardKit'

// Shared channel map — same source, same colour on every page (DESIGN_SYSTEM.md).
const SRC_COLORS = SOURCE_COLORS
    ...
    <div className={styles.layout}>
      ...
      <div className={styles.main}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:12, marginBottom:20 }}>
          <KPICard label="AC Revenue"    value={fmtR(totals.total_ac)}    sub="Admission Counselling"  accent={C_KPI.green} accentBg={C_KPI.greenBg}
          ...
          <KPICard label="VAS Revenue"   value={fmtR(totals.total_vas)}   sub="Value Added Services"   accent={C_KPI.blue}  accentBg={C_KPI.blueBg}
          ...
          <KPICard label="Total Revenue" value={fmtR(totals.total_rev)}   sub="AC + VAS Collected"     accent={C_KPI.cyan}  accentBg={C_KPI.cyanBg}
          ...
          <KPICard label="Projected"     value={fmtR(totals.total_proj)}  sub="Total Package Value"    accent={C_KPI.navy}  accentBg={C_KPI.navyBg}
          ...
          <KPICard label="Collection %"  value={totals.collection_pct+'%'} sub="Collected / Projected" accent={parseFloat(totals.collection_pct)>=50?C_KPI.green:C_KPI.navy} accentBg={parseFloat(totals.collection_pct)>=50?C_KPI.greenBg:C_KPI.navyBg}
          ...
          <KPICard label="Unidentified"  value={fmtR(totals.unid_rev)}    sub={`${totals.unid_pct}% of total`} accent={C_KPI.muted} accentBg="#F3F4F6"
          ...
          <div className={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={acVasMonthly} margin={{top:4,right:8,left:0,bottom:0}} barCategoryGap="25%"><defs><linearGradient id="gradRev" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#1C9FD4" stopOpacity={0.15}/><stop offset="95%" stopColor="#1C9FD4" stopOpacity={0}/></linearGradient></defs>
                <defs><BarGrad id="g-rev-ac" color={C.green}/><BarGrad id="g-rev-vas" color={C.blue}/></defs>
                ...
                <Bar dataKey="AC"  name="AC Revenue"  fill={barFill('g-rev-ac')}  radius={BAR_RADIUS} maxBarSize={BAR_MAX}/>
                <Bar dataKey="VAS" name="VAS Revenue" fill={barFill('g-rev-vas')} radius={BAR_RADIUS} maxBarSize={BAR_MAX}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className={styles.card}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={collectionMonthly} margin={{top:4,right:8,left:0,bottom:0}} barCategoryGap="25%">
                <defs><BarGrad id="g-rev-coll" color={C.green}/></defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" vertical={false}/>
                <XAxis dataKey="month" tick={{fontSize:12,fill:'#9CA3AF'}} axisLine={false} tickLine={false}/>
                <YAxis tick={{fontSize:12,fill:'#9CA3AF'}} axisLine={false} tickLine={false} tickFormatter={v=>fmtR(v)} width={75}/>
                <Tooltip content={<BrandTooltip/>}/>
                <Legend wrapperStyle={{fontSize:12.5}}/>
                <Bar dataKey="Collected"  fill={barFill('g-rev-coll')} radius={BAR_RADIUS} maxBarSize={BAR_MAX}/>
                <Bar dataKey="Projected"  fill={NEUTRAL_TRACK} radius={BAR_RADIUS} maxBarSize={BAR_MAX}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
                        ...
                        {cp>0?(
                          <div style={{display:'flex',alignItems:'center',gap:6}}>
                            <div style={{flex:1,background:'#F3F4F6',borderRadius:3,height:6,overflow:'hidden'}}>
                              <div style={{width:`${Math.min(cp,100)}%`,height:'100%',background:parseFloat(cp)>=50?C.green:C.navy,borderRadius:3}}/>
                            </div>
