...
function QuantumIcon({ dark = true }) {
  return (
...
export default function LoginPage() {
  ...
  // sign-in logic (GoogleLogin -> handleGoogleSuccess -> useAuth) only exists
  // once, no matter which visual layout is selected. `dark` picks the Google
  // button's own theme so it still reads correctly against a light or dark
  // background (this is Google's own rendered widget -- its internal colors
  // can't be restyled beyond the theme/shape/size props it already exposes).
        ...
        <div className={styles.lModalScene}>
          ...
          <div className={styles.lModalOverlay}>
            <div className={styles.lCard}>
              {brandLogo(false)}
              <h2 className={styles.lHeading}>Sign in to continue</h2>
              <p className={styles.lSub}>Your dashboards are ready.</p>
              {errorBlock(true)}
              {googleBlock(false)}
            </div>
          </div>
